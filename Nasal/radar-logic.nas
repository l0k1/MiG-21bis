...
# radar power settings - known distance, and known rcs detectable at that distance.
# if in doubt, us maximum published figure and an known rcs of 5.
var radar_known_distance = 80;
var radar_known_rcs = 5;

var isVisibleToRadar = func(SelectCoord,node) {
	var isVisible = 0;
	var my_heading = getprop("/orientation/true-heading-deg");
	var my_pitch = getprop("/orientation/pitch-deg");
    var distance = SelectCoord.direct_distance_to(geo.aircraft_position);
    var target_model = node.getNode("name"); # get model name
    var target_heading = node.getNode("orientation/true-heading-deg");
    var target_pitch = node.getNode("orientation/pitch-deg");
    var target_roll = math.abs(node.getNode("orientation/roll-deg"));
    var rel_heading = func(target_heading, my_heading) { math.abs(target_heading - my_heading) > 180 ? math.abs((my_heading + 360) - target_heading) : math.abs(target_heading - my_heading) }
    rel_heading = math.abs(rel_heading);
	var abs_pitch = math.atan2(aircraftAlt-myAlt, distance);
	var rel_pitch = math.abs((abs_pitch - my_pitch) + target_pitch);
	...
	if ( aircraft_rcs_database[target_model] != nil ) {
		var rcsVals = aircraft_rcs_database[target_model].get_all_rcs;
	} else {
	...
	var currMaxDist = knownDist/math.pow(knownRCS/currRCS, 1/4);
...
var aircraft_rcs = {
    new: func(rcs_front, rcs_side = 0, rcs_back = 0, rcs_top = 0) {
        var ac = { parents : [Contact]};}
    	ac.rcs_front = rcs_front;
    	ac.rcs_side  = func(rcs_front, rcs_side) { rcs_side != 0 ? rcs_side : rcs_front * 2.50 }
    	ac.rcs_back  = func(rcs_front, rcs_back) { rcs_back != 0 ? rcs_back : rcs_front * 1.75 }
    	ac.rcs_top   = func(rcs_front, rcs_top) { rcs_top != 0 ? rcs_top  : rcs_front * 3.50 }
        return ac;
    },
    get_rcs_front: func(){
    	return me.rcs_front;
    ...
    get_all_rcs: func(){
    	return [rcs_front, rcs_back, rcs_side, rcs_top];
    }
}

var aircraft_rcs_database = {
	"Grumman F-14 Tomcat": 		 20.0,
	"F-15C": 					 15.0,
	"F-15D": 					 15.0,
	"JA37-Viggen": 				  2.0,
	"AJ37-Viggen": 				  2.0,
	...
	"Blackbird-SR71B": 			  0.01,
	"Blackbird-SR71A-BigTail": 	  0.01,
	"ch53e": 					 25.0, # unknown - probably higher than this
	"MiG-21bis": 				  3.0,
	"MQ-9": 					  1.0, # unknown - probably lower than this
	"KC-137R": 					100.0,
	"KC-137R-RT": 				100.0,
	"A-10 Thunderbolt II": 		 10.0, # unknown
	"KC-10A": 					 90.0,
	"Typhoon": 					  0.5,
	"C-137R": 					100.0,
	"E-8R": 					100.0,
	...
	"UH-60J": 					 15.0, # unknown
	"uh1": 						 15.0, # unknown
	"212-TwinHuey": 			 15.0, # unknown
	"412-Griffin": 				 15.0, # unknown
	"QF-4E": 					 15.0, # unknown
}